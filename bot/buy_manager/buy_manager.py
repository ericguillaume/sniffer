...
class BuyManager():
  ...
  def buy(self, symbol):
    sbm = SymbolBuyManager(self.price_manager, symbol, self.keep_for_k_minutes, buy_price)
    sbm.buy()
...
class SymbolBuyManager(threading.Thread):
  ...
  def buy(self):
    # buying
    success, buy_price, _ = price_manager.get_current_symbol_price(symbol) # try twice
    if not success:
      return
  ...
  def run(self):
    ...
    sell_time = time.time()
    success, sell_price, _ = price_manager.get_current_symbol_price(self.symbol)  # try twice
    if not success:
  ...
  def time_was_updated(self):
    ...
    sell_price = self.prices_manager.get_last_price(self.symbol)
